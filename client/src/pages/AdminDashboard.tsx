...
const AdminDashboard: React.FC = () => {
    ...
    useEffect(() => {
        const fetchDashboard = async () => {
            try {
                ...
                const response = await fetch(`${config.BASE_URL}/admin/dashboard`, {
                    method: 'GET',
                    headers: {
    ...
    useEffect(() => {
        const fetchAnalytics = async () => {
            ...
            try {
                const salesResponse = await fetch(`${config.BASE_URL}/api/analytics/sales`, {
                    headers: { 'Authorization': `Bearer ${token}` },
                });
                const sales = await salesResponse.json();
                setSalesData(sales);

                const inventoryResponse = await fetch(`${config.BASE_URL}/api/analytics/inventory`, {
                    headers: { 'Authorization': `Bearer ${token}` },
                });
                const inventory = await inventoryResponse.json();
                setInventoryData(inventory);

                const comparisonResponse = await fetch(`${config.BASE_URL}/api/analytics/product-comparison`, {
                    headers: { 'Authorization': `Bearer ${token}` },
                });
